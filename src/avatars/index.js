import info from './avatars.json'

const url = "/neoboards/avatars/?username="
...
function handler() {
    ...
    for (let image of images) {
        ...
        const i = info.findIndex(e => e.url === image.src || e.avatar === image.title)
        if (i > -1) {
            info[i].done = done
        } else {
    ...
    <section id="userscript-avatars">
        ...
        <table>
            ...
            <tbody>
                ${
                    info.map(e => `<tr data-done="${e.done}" data-retired="${e.retired}">
                        <td><img src="${e.url}" /></td>
                        <td>${e.avatar}</td>
                        <td>${e.type}</td>
                        <td>${e.method}</td>
